            ...
            for sector_name in obj.sectors:
                for impact_name in obj.climate_impacts:
                    ...
                    if index_name not in edges_index:
                        ...
                        edges.append(
                            {
                                "from": sectors[sector_name],
                                "to": impacts[impact_name],
                                "value": 0,
                            }
                        )
        ...
        for brain in brains:
                    ...
                    if hasattr(ao, "ipcc_category"):
                        for ipcc_category in ao.ipcc_category:
                            ...
                            if ipcc_category not in index_ipcc_categories:
                                ...
                                nodes.append({"id": ipcc_category.encode("utf-8")})
                except:
                    ""
            ...
            for sector_name in obj.sectors:
                ...
                if sector_name not in list(index_sectors.keys()):
                    ...
                    nodes.append({"id": sector_name.encode("utf-8")})
            for impact_name in obj.climate_impacts:
                ...
                if impact_name not in list(index_impacts.keys()):
                    ...
                    nodes.append({"id": impact_name.encode("utf-8")})
            ...
            for sector_name in obj.sectors:
                for impact_name in obj.climate_impacts:
                    ...
                    if index_name not in edges_index:
                        ...
                        edges.append(
                            {
                                "source": sector_name.encode("utf-8"),
                                "target": impact_name.encode("utf-8"),
                                "value": 0,
                            }
                        )
                ...
                for impact_name in obj.climate_impacts:
                    ...
                    for ipcc_category in list_ipcc_categories:
                        ...
                        if index_name not in edges_index:
                            ...
                            edges.append(
                                {
                                    "source": impact_name.encode("utf-8"),
                                    "target": ipcc_category.encode("utf-8"),
                                    "value": 0,
                                }
                            )
...
class Plotly(BrowserView):
    ...
    def compute(self):
        ...
        for brain in brains:
            ...
            tag = lxml.etree.fromstring(
                "<p>"
                + obj.long_description.raw.encode("utf-8").decode("utf-8")
                + "</p>"
            )
            ...
            item = {
                ...
                "url": brain.getURL(),
                "sectors": [
                    vocabulary_sectors.getTerm(name).title for name in obj.sectors
                ],
                "impacts": [
                    vocabulary_impacts.getTerm(name).title
                    for name in obj.climate_impacts
                ],
            ...
            for sector_name in obj.sectors:
                ...
                for impact_name in obj.climate_impacts:
                    ...
                    if index_name not in edges_index:
                        edges_index[index_name] = len(edges)
                        edges.append(
                            {"source": sector_name, "target": impact_name, "value": 0}
                        )
            ...
            for impact_name in obj.climate_impacts:
                ...
                for ipcc_category in list_ipcc_categories:
                    ...
                    if index_name not in edges_index:
                        edges_index[index_name] = len(edges)
                        edges.append(
                            {"source": impact_name, "target": ipcc_category, "value": 0}
                        )
