...
from z3c.relationfield import RelationValue, event
...
class InspectRelationsControlpanel(BrowserView):
    def __call__(self, relation=None, inspect_backrelation=False):
        ...
        self.relations = []
        self.relations_stats, self.broken = get_relations_stats()
        # view_action = api.portal.get_registry_record(
...
def store_relations(context=None):
    ...
    logger.info("Stored {0} relations on the portal".format(len(all_relations)))
...
def export_relations(context=None):
    ...
    with open("all_relations.json", "w") as f:
        ...
        logger.info(
            "Stored {0} relations as all_relations.json".format(len(all_relations))
        )
...
def restore_relations(context=None, all_relations=None):
    ...
    if len(unique_relations) < len(all_relations):
        logger.info(
            "Dropping {0} duplicates".format(len(all_relations) - len(unique_relations))
        )
...
def backrelations(obj, attribute=None, as_dict=False):
    ...
    return get_relations(
        obj, attribute=attribute, backrels=True, restricted=True, as_dict=as_dict
    )
...
def unrestricted_backrelations(obj, attribute=None, as_dict=False):
    ...
    return get_relations(
        obj, attribute=attribute, backrels=True, restricted=False, as_dict=as_dict
    )
