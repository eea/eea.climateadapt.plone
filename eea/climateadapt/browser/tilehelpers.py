...
class AceContentSearch(BrowserView):
    ...
    def items(self):
        return self.parent.getFolderContents(
            {"portal_type": "Event", "sort_by": "effective"}, full_objects=True
        )[:3]
...
class FrontPageCountries(BrowserView):
    ...
    def countries(self):
        ...
        countries = [
            c.getObject()
            for c in countries_folder.getFolderContents(
        ...
        res = [
            (c.getId(), c.Title())
            for c in countries
            if c.portal_type in ["Folder", "collective.cover.content"]
        ]
