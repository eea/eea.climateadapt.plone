...
from eea.climateadapt.testing import EEA_CLIMATEADAPT_INTEGRATION_TESTING
...
class IntegrationTest(unittest.TestCase):
    ...
    def test_schema_publication_report(self):
        fti = queryUtility(IDexterityFTI, name="eea.climateadapt.publicationreport")
        schema = fti.lookupSchema()
        self.assertEqual(IPublicationReport, schema)

    def test_add_publication_report(self):
        self.portal.invokeFactory(
            "eea.climateadapt.publicationreport", "publication_report"
        )
        self.assertTrue(IPublicationReport.providedBy(self.portal.publication_report))

    def test_schema_information_portal(self):
        fti = queryUtility(IDexterityFTI, name="eea.climateadapt.informationportal")
        schema = fti.lookupSchema()
        self.assertEqual(IInformationPortal, schema)

    def test_add_information_portal(self):
        self.portal.invokeFactory(
            "eea.climateadapt.informationportal", "information_portal"
        )
        self.assertTrue(IInformationPortal.providedBy(self.portal.information_portal))

    def test_schema_guidance_document(self):
        fti = queryUtility(IDexterityFTI, name="eea.climateadapt.guidancedocument")
        schema = fti.lookupSchema()
        self.assertEqual(IGuidanceDocument, schema)

    def test_add_guidance_document(self):
        self.portal.invokeFactory(
            "eea.climateadapt.guidancedocument", "guidance_document"
        )
        self.assertTrue(IGuidanceDocument.providedBy(self.portal.guidance_document))
    ...
    def test_add_organisation(self):
        self.portal.invokeFactory("eea.climateadapt.organisation", "organisation")
    ...
    def test_add_casestudy(self):
        self.portal.invokeFactory("eea.climateadapt.casestudy", "case_study")
        self.assertTrue(ICaseStudy.providedBy(self.portal.case_study))

    def test_schema_adaptationoption(self):
        fti = queryUtility(IDexterityFTI, name="eea.climateadapt.adaptationoption")
        schema = fti.lookupSchema()
        self.assertEqual(IAdaptationOption, schema)

    def test_add_adaptationoption(self):
        self.portal.invokeFactory(
            "eea.climateadapt.adaptationoption", "adaptation_option"
        )
        self.assertTrue(IAdaptationOption.providedBy(self.portal.adaptation_option))
    ...
    def test_add_aceproject(self):
        self.portal.invokeFactory("eea.climateadapt.aceproject", "ace_project")
        self.assertTrue(IAceProject.providedBy(self.portal.ace_project))
