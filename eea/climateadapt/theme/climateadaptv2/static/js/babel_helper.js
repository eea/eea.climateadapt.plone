...
(function ($) {
  ...
  function sync_element_vertically(original, destination, padding, first) {
    ...
    $([original, destination]).each(function (index, item) {
      var $item = $(item),
        outer_padding = 0,
        parent = $item.parent();
      outer_padding = Math.max(
        $item.position().top +
          $item.height() -
          (parent.position().top + parent.height()) +
          padding,
        0
      );
      if (outer_padding) {
  ...
  function sync_elements_vertically() {
    ...
    $.each(original_fields, function (i) {
      sync_element_vertically(
        $(original_fields[i]),
        $(destination_fields[i]),
        padding,
        i === 0
      );
    });
  ...
  function update_view() {
    ...
    if (original_fields.length > 1) {
      padding =
        $(original_fields[1]).position().top -
    ...
    $.each(original_fields, function (index, value) {
      ...
      if (
        $("#gtranslate_service_available").attr("value") === "True" &&
        (original_field.find(
          ".richtext-field, .textline-field, .text-field, .localstatic-field, .ArchetypesField-TextField"
        ).length > 0 ||
          $("#at-babel-edit").length > 0)
      ) {
        original_field.prepend(
          "<div class='translator-widget' id='item_translation_" +
            order +
            "'></div>"
        );
